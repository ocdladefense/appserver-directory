...
import QueryBuilder from "/node_modules/@ocdladefense/node-query-builder/QueryBuilder.js"; // import {FileUploadService,FileUploadComponent} from "/node_modules/@ocdladefense/node-file-upload/Upload.js";
...
function showMap() {
  ...
  window.myMap = myMap;
  qb.render("custom"); // Listen for changes to the underlying query UX.

  document.addEventListener("querychange", contactQuery, true);

  function contactQuery(e) {
    var query = e.detail;
    var searchFeature = myMap.getFeature("search"); //need to clear markers?

    searchFeature.setDatasource(doSearch.bind(null, query)); // Load the feature's data.

    searchFeature.loadData(); // Load the feature's markers.

    searchFeature.loadMarkers().then(function () {//show the feature
  ...
  myMap.init().then(function () {
    ...
    myMap.render(ocdlaIcon); // Set up the features and load in the data
