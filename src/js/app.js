...
function doQueryChange(e) {
	...
	.then(function(result) {
		...
		component.update(<Main entries={members} page={page} limit={query.limit} count={result.count} activeRecord={id} />);
...
function updateView(qb) {
	...
	.then(function(result) {
		...
		component.render(<Main entries={members} page={1} limit={query.limit} count={result.count} />);
		// updateView();
...
function updateMap(members) {
	...
	search: {
		...
		datasource: function(){ return Promise.resolve(members); }
